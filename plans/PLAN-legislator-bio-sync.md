...
**Status:** Phase 1 in progress — steps 1, 2, 3a, 3b, 4, 5, 6, 7 implemented (commits 2852a36, 24d058e, c555f38, ee74124, b37e408, 8263902, e1834f8, 4427dd3, + forthcoming round-10 follow-up commit); 80-test suite landed. Ten pm-review rounds folded in (rev 11). Steps 8–9 still pending.
**Created:** 2026-04-29
**Repo:** ddp-sync
...
| 7 | Run-summary alerting via Zapier | 4427dd3 + round-10 follow-up | ✅ `push_bio_sync_alert(webhook_url, report, *, large_changes_threshold=100)` mirrors `voatz_brevo.push_alert_to_zapier`. Wired into `LegislatorBioPipeline.run()` via `try/finally`; alert fires on **every non-dry-run completion including aborts**. Payload includes `on_failure` and `on_large_changes` threshold flags + the threshold value itself for Zapier-side routing. **Round-9 follow-ups:** jurisdiction cache 1h TTL + stale-on-empty-refresh reuse + `metric=webflow.jurisdiction_mapping_empty` breadcrumb. **Round-10 follow-ups:** `asyncio.Lock` serializes concurrent refresh attempts; `large_changes_threshold` extracted to a named constant + tunable parameter; `metric=legislator_bio_sync.alert_sent` on successful POST closes the SLA-dashboard gap; verified `settings.zapier_webhook_url` ↔ `ZAPIER_WEBHOOK_URL` wiring matches the existing `voatz_brevo` pattern. |
| 7a | Alerting + cache test suite | 4427dd3 + round-10 follow-up | ✅ 6 alert-function tests (no-webhook, 200 OK with payload assertions, aborted-run flag, on_large_changes default threshold, non-2xx, exception); 4 `run()` integration tests; 4 cache tests (TTL, stale-reuse on empty, empty-mapping breadcrumb, lock-serializes-concurrent-refresh); 2 round-10 tests (threshold-tunable, success-metric-emitted). **Total suite: 80 tests, all pass.** |
