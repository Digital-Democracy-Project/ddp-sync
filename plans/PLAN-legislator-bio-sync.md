...
**Status:** Phase 1 in progress — steps 1, 2, 3a, 3b, 4, 5, 6, 7, 8 implemented (commits 2852a36, 24d058e, c555f38, ee74124, b37e408, 8263902, e1834f8, 4427dd3, c492891, 507adaa, a0d2300, + forthcoming round-13 follow-up commit); 93-test suite landed. Thirteen pm-review rounds folded in (rev 14). Step 9 still pending.
**Created:** 2026-04-29
**Repo:** ddp-sync
...
| 8 | Orchestrator-internal `run()` integration tests | a0d2300 + round-13 follow-up | ✅ `tests/test_legislator_bio_orchestrator.py` — 11 integration tests covering full-pass `run()` flows: (1) federal happy path via OpenStates with bioguide-enrichment, (2) departed-federal bioguide-fallback (Karen Bass case), (3) state record skipped as Phase-2 stub (logged + not orphaned + no PATCH), (4) dry-run emits no PATCHes and no Zapier alert, (5) per-record WebflowError continues run with errors[] entry, (6) WebflowRateLimitError aborts run cleanly with alert still firing, (7) jurisdiction="us" filter excludes state records, (8) jurisdiction-cache lock releases when fetch raises. **Round-13 additions:** (9) `locked_fields` integration test — fields named in `BioSyncOptions.locked_fields` are excluded from PATCH even when upstream differs; (10) `large_changes_threshold` end-to-end with 101 federal records → real Zapier payload reflects `on_large_changes=True`; (11) mixed-success PATCHes — 2 succeed + 1 fails with WebflowError, run continues, report records 2 successes + 1 error. `_build_pipeline()` fixture uses `MagicMock(spec=...)` on service mocks so attribute drift on `WebflowLookupService` / `OpenStatesPeopleClient` / `CongressLegislatorsSource` raises at access time. |
| 8a | Round-12 + round-13 polish bundled into step 8 | a0d2300 + round-13 | ✅ Doc-comment on `get_jurisdiction_mapping` clarifies returned dict is read-only. Lock-release-on-raising-fetch test pins the contract. **Round-13:** integration coverage for the three safety-valve paths the reviewer flagged as gaps before step 9 (locked_fields, large_changes_threshold alert, mixed-success PATCHes). `MagicMock(spec=...)` tightening catches future signature drift. |
...
8. **Orchestrator-internal `run()` integration tests — IMPLEMENTED:**
   ...
   d. ✅ Round-13 safety-valve integration tests: `locked_fields` excludes named fields from PATCH; `large_changes_threshold` end-to-end with 101 records → real Zapier payload reflects `on_large_changes=True`; mixed-success PATCHes (2 succeed + 1 WebflowError) records per-record results correctly. `MagicMock(spec=...)` tightening on service mocks catches future signature drift. **Total suite: 93 tests, all pass.**
