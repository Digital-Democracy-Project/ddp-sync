...
@pytest.mark.asyncio
async def test_http_status_error_returns_cams_error():
    mock_error_response = MagicMock(status_code=503, text="Notion scraper unavailable")
    mock_response = MagicMock()
    mock_response.raise_for_status = MagicMock(
        side_effect=httpx.HTTPStatusError(
            "503", request=MagicMock(), response=mock_error_response,
        )
    )
    mock_post = AsyncMock(return_value=mock_response)
    cm = _mock_client(mock_post)
...
@pytest.mark.asyncio
async def test_request_error_returns_cams_unreachable():
