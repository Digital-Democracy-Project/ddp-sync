...
- **OpenStates scrape jobs** (managed via `openstates_scrape` block in `sync_schedule.yaml`):
  ...
  - **Sustained-blocking escalation** (OPEN-22, 2026-08-02): the per-run alert above treats a first-time WAF block and a month of consecutive blocks identically — nothing previously distinguished "one bad week" from "MI has been blocked for a month straight". Every secondary-jurisdiction run now gets classified (`classify_failure_reason()` — `waf_block` / `timeout` / `network_error` / `nonzero_exit_other`, matched against this codebase's own first-party `ScrapeError` message text from `openstates-scrapers`' `MIWafCircuitBreakerMixin`, OPEN-18/OPEN-22 AC7) and appended to a per-jurisdiction rolling history in Redis (`RedisStore.append_run_history`/`get_run_history` — a capped list under its own `ddp:flow_history:` key, deliberately additive alongside `set_flow_status`/`get_flow_status` rather than changing that shared primitive, which has several other callers). `should_escalate()` is a pure, stateless function — no separate streak counter to keep in sync — that checks whether at least `secondary.escalation.threshold` of the jurisdiction's last `secondary.escalation.window_size` runs (default 3 of 4) were WAF-block-classified, and fires a distinctly-worded Slack alert (`_alert_sustained_block()`, same `#automation-errors` channel) separate from the per-run failure alert if so. Jurisdiction-agnostic (keyed by jurisdiction code, not hardcoded to MI) — today only MI ever produces a `waf_block` classification, but any future jurisdiction with the same failure mode is covered automatically. Deliberately does **not** skip or delay a scheduled run based on this history (unproven value given MI already only runs weekly, and skipping means less data on whether a block has actually cleared) — visibility/escalation only. `AC0/AC0b/AC1-AC6` of OPEN-22; the ticket's `AC7` (MI-events WAF circuit-breaker parity) shipped separately in `openstates-scrapers`. **This classification never actually fired in production until SYNC-3 (2026-08-31):** `classify_failure_reason()` only ever inspected the scrape's external stdout/stderr, but a real `WafBlockDetected` signature is written only to `logs/scraper.log` — redirected there inside `run-scrape.sh`'s own `tee` pipeline, never reaching the external stderr this classifier could see — so every genuine block classified as generic `nonzero_exit_other` and the escalation above could never accumulate the `waf_block` records it counts. Fixed by additionally reading a bounded tail of `scraper.log`, anchored to a byte offset captured *before* the subprocess launches (not a fixed line/byte count from the end) — the anchor matters because `scraper.log` is a long-lived, cross-run file, so an unanchored read could misattribute a stale, already-resolved block from a prior run to today's unrelated failure.
  - **OpenStates archive job** (`openstates_archive` block in `sync_schedule.yaml`, daily 05:00 UTC per jurisdiction — fl/ut/az/wa/va/mi/ma/al/us): archives each jurisdiction's bill-version documents via `os-text-extract archive <state>`, run through `run-archive.sh`. Overlap with a same-jurisdiction scrape is safe by design (the natural-key skip check makes an already-archived version a cheap DB check, not a re-fetch — OPEN-107 fixed a case where two archivers racing on that check turned a benign lost race into a run-failing exit). **Timeout/orphan-process parity with the scrape path** (SYNC-2, 2026-08-31): `_run_archive()`'s subprocess timeout previously killed only the bash wrapper on `TimeoutExpired`, leaving `os-text-extract archive <state>` and its `tee` running orphaned in the detached session — discovered live 2026-08-07 when a timed-out MI archive run's orphaned process ran ~24h unsupervised, and the wrapper's own `ERR` trap (which owned Slack/CAMS alerting) died with the wrapper, so the whole incident alerted nobody. Now reuses `openstates_scrape.py`'s `_run_with_group_kill()` to kill the whole process group on timeout, and gained its own `_alert_archive_failure()` (Slack `#automation-errors` + CAMS `/api/v1/failures`, mirroring `_alert_scrape_failure()`) for the timeout/signal-kill/pre-subprocess-exception paths — deliberately *not* for an ordinary nonzero exit, since `run-archive.sh`'s own in-process trap already alerts on that case and group-kill doesn't touch it. MI's own `ARCHIVE_TIMEOUT_S` raised from the 4h default to 24h to actually clear its ~8k-document backlog (~10s/doc) instead of getting killed partway through every night.
...
| `LEGBOT_DISPATCH_TIMEOUT_SECONDS` | `1200` | AGENTS-42: now specifically the *inference* timeout — how long `_dispatch_and_await`'s poll loop waits for a task to finish once `ddp-agents`' CAMS reports its `mlx_generation_started_at` marker as populated (real MLX generation has begun), measured from that moment. Raised 2026-08-15 from a hardcoded 120s — too short for an MLX-routed call under real pool contention (LegBot serializes all MLX-bound calls, see `ddp-agents` CLAUDE.md's LegBot section). On timeout, this client now also sends a best-effort `DELETE /api/v1/tasks/{task_id}` to CAMS so the abandoned task doesn't keep running as an orphan (SYNC-20) — a cancel failure is logged but never masks the original timeout error. |
...
**2026-09-11 (OPEN-275/OPEN-276): a pre-dispatch replica-freshness check exists but is not live.** ddp-infra's
`PLAN-rds-local-postgres-replication.md` sets up Postgres logical replication from RDS into the Mac's own local
Postgres, so LegBot dispatch could read RDS-loaded (cloud-owned-jurisdiction) bills through the Mac's *existing*
local read path instead of a separate RDS-facing service. `services/replica_freshness.py`'s
`check_bill_version_freshness()` (a per-bill, fail-closed content-hash comparison against RDS) and a jurisdiction
allowlist (`LEGBOT_RDS_REPLICA_JURISDICTION_ALLOWLIST`) are wired into `_process_bill_inner`, both gated behind
`REPLICA_FRESHNESS_CHECK_ENABLED` (default `false`). **Neither is usable yet**: RDS's own `wal_level` is still
`replica`, not `logical` (OPEN-271), so the replication subscription this depends on cannot exist until that
lands. This targets the same "how does LegBot dispatch see RDS-loaded data" problem SYNC-59's own
`RDS_OPENSTATES_API_BASE` setting also anticipated, via a different mechanism (replication vs. a second api-v3
instance) — the two have not been reconciled; see CLAUDE.md before building on either.
...
**Updated 2026-08-31 (SYNC-45): the gap above is now closed.** The per-bill coverage gate escalates
past the cheap single-version check specifically for `bill_changelog` — coverage now means *every
classified version after the oldest holds a changelog*, read via `get_bill_artifact_coverage_all_versions`
(BROKER-130's `?versions=all`). The escalation only fires when the single-version check already says
"covered", which is exactly where the miss hides — a bill whose latest version has no changelog at all
still takes the original dispatch branch, paying no extra broker read. The six FL 2026E bills above now
get dispatched for their missing `Filed`→`e1` transition instead of being permanently skipped; their
existing `e1`→`er` changelog is left alone (it's a real analysis of a real, if trivial, diff). A broker
predating BROKER-130 is treated as "cannot tell, so covered" rather than "not covered" — the safe
direction, since the alternative would re-dispatch every bill in a session on every run against an old
broker.
