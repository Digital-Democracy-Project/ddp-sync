...
| `LEGBOT_POLL_INTERVAL_SECONDS` | `1` | SYNC-39: how often `_dispatch_and_await` asks CAMS whether a task has finished. Was a hardcoded `5`, and that number — not anything about scheduling — is why bills kept losing their prefilled MLX cache. A bill does not learn its own call finished for up to one interval, then spends roughly another on a broker write and a coverage check before dispatching again; its worker sits idle that whole time while the bill still needs it, and every competing bill is polling on the same rhythm, so one of them claims it. Measured on VA 2026S1 (2026-08-27): a bill left a ~10s window against a competitor cadence of ~5s, and 17 of 22 warm workers were taken within 5s of going idle — 13 of 18 cold `concept_statements` prefills were exactly this. `1` puts the window under the cadence instead of over it. More polls against a service on the same machine costs nothing next to the 15–30s prefill each lost cache costs. `scrapebot_client.py` keeps its own `5` deliberately: its mint holds no per-bill cache. |
