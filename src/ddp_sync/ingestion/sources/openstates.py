...
class OpenStatesSource:
    ...
    async def fetch(
        ...
        **kwargs,
        ...
        # and it's listed in settings.ddp_openstates_jurisdictions. With no
        # jurisdiction (multi-jurisdiction crawl), there's nothing to route
        # on, so this always uses the public API.
        if jurisdiction:
            api_base, api_key, is_local_replica = self._get_api_base_and_key(jurisdiction)
        else:
    ...
    async def fetch_legislator_by_id(
        ...
        # is available here to route to the local replica, so this always
