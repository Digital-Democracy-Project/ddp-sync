...
async def get_all_versions(bill_openstates_id: str) -> list[dict] | None:
    """Fetch every version api-v3 has recorded for a bill (SYNC-26) --
    the same single-bill detail fetch get_current_version_identity/
    get_archived_bill_text already use, just returning the full
    ``versions`` array instead of deriving one field off it. Needed
    because ddp-sync's own BillVersion ledger in ddp-broker-py only ever
    gets a row for whichever single version a bill happened to be at the
    first time ddp-sync's polling job saw it -- a fast-moving bill that
    already had several versions by its first sighting silently never got
    ledger rows for the earlier ones, which later breaks bill_changelog's
    compare_version FK resolution even though api-v3 itself has every
    version and diff archived correctly. This is the read half of both the
    one-off backfill (scripts/backfill_bill_versions.py) and the standing
    fix (BillVersionSyncService._backfill_missing_versions).

    Returns:
        The bill's ``versions`` list (each ``{"date", "note", "links": [...]}``,
        ascending, latest last per OPEN-92's stage-aware ordering) if the
        bill has at least one version archived, else None -- same
        never-abort posture as this module's other lookups: a caller that
        can't resolve this for one bill should skip that bill, not abort
        the whole batch.
    """
    settings = get_settings()
    if not settings.local_openstates_api_base:
    ...
    try:
        ...
        logger.warning(
            "Local api-v3 unreachable -- cannot resolve bill versions",
            bill_openstates_id=bill_openstates_id,
            error=str(exc),
        )
        return None
    ...
    return data.get("versions") or None
